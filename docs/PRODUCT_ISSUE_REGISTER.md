...
| DAT-002 | P0 | Data | Move facility search pagination and map bounds server-side. | Prototype implementation complete: `/api/facilities/search` supports page, page size, offset, sort, filters, radius, and map bounding box without loading 50k records to the browser. Production auth enforcement remains under `SEC-001`. |
